%% Brainstorm LCMV beamforming pipeline for Simulated MEG data
...
data_dir = '/net/qnap/data/rd/ChildBrain/Simulation/NEW_25_seglabBEM6/'; % remove it
...
%% Set parameters
par                 = [];
par.Baseline        = [-0.200 -0.020];
par.DataWindow      = [0.020 0.200];
par.sampling_rate   = 1000; 
par.stimchan        = 'STI101';
par.bads            = 'MEG2233, MEG2212, MEG2332, MEG0111';
par.more_plots      = 0;
par.bpfreq          = [2 40];
par.gridres         = 5; % mm
par.cov_cut         = [2, 98];
...
%% Run to analyze all data (select i_file for specific data)
for i_file = 1:400
    
    %% ===== CREATE PROTOCOL =====
    ProtocolName = 'Simulated';
    ...
    % Phantom anatomy
    SubjectName = ['Simu_' date];


    % Load all filenames
    load([code_dir 'simulated_filenames.mat'])
    AnatDir  = [mri_dir 'BeamCompMRI'];
    data_path = data_dir;
   ...
   %% Process: Import anatomy folder from Freesurfer
    ...
    sFiles = bst_process('CallProcess', 'process_evt_rename', sFiles, [], ...
        'src',  num2str(eventval), ...
        'dest', sprintf('%02d ',eventval));
    
    eventlabel = sprintf('%02d ',eventval);
    fprintf('\neventlabel = %s\n', eventlabel)
    ...
    %% Plot to view time series
     NM_bst_view_dataplots(par, sFilesAvg, 10, par.more_plots)
    ...
    difff = sqrt(sum((act_loc-Est_locNM).^2));
    disp(fix([Est_locNM, Est_val, difff, n_act_grid, PSVol]))    
    
    diary([code_dir 'diary_BST_Simulated_results.txt'])
    fprintf('%s,\t %.1f, %.1f, %.1f,\t %.1f, \t%.1f, \t%d, \t%d \n', dfname, Est_locNM, Est_val, difff, n_act_grid, PSVol)
    diary('off')
