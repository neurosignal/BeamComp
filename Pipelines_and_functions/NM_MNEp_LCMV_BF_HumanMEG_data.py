...
par={'badch'      : [],
    'stimch'     : 'STI 014'}
code_dir = '/home/amit/git/ChildBrain/BeamComp/MATLAB/'
data_dir = '/net/qnap/data/rd/ChildBrain/'
...
act_dip_ = loadmat(code_dir + 'BeamComp_CodeRepo/LCMV_pipelines/multimodal_biomag_Xfit_results.mat')
...
data_path = data_dir + 'BeamComp_DataRepo/MEG/Human_EF/'   
filename  = 'multimodal_raw.fif'   # or, multimodal_raw_tsss.fif    
fname     = data_path + filename
subjects_dir, subject = data_dir + 'BeamComp_DataRepo/MRI/', 'BeamCompMRI'
...
raw.filter(2, 95, l_trans_bandwidth=min(max(2 * 0.01, 2), 2), 
           h_trans_bandwidth=min(max(95 * 0.01, 2.), raw.info['sfreq'] / 2. - 95), 
           filter_length='auto', phase='zero', picks=None)
