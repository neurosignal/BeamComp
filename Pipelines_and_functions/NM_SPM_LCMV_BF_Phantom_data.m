...
restoredefaultpath 
restoredefaultpath
spm_dir  = '/BeamComp/MATLAB/BeamComp_CodeRepo/spm12/';
...
%%
for datacats={'Static_phantom', 'Moving_phantom'}
    
    datacat = char(datacats);
    ...
    if isequal(datacat, 'Static_phantom')
        ...
        fname =[data_path 'Amp' par.sub{1,amp} '_IASoff/' 'Amp' par.sub{1,amp} '_Dip' num2str(dip) '_IASoff' par.prep{1,pp} '.fif'];

    elseif isequal(datacat, 'Moving_phantom')
        data_path= [data_dir 'MEG/Phantom/' datacat '/'];
        fname =[data_path 'Amp' par.sub{1,amp} '_IASoff_movement/' 'Amp' par.sub{1,amp} '_Dip' num2str(dip) '_IASoff_movement' par.prep{1,pp} '.fif'];
    end
    ...
    clearex par datacats datacat channelslist actual_diploc spm_dir mri_dir data_dir ...
            code_dir work_dir pp maxf amp amps dip dipoles phantom_moving 
            end
        end
    end
end
