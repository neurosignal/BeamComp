...
restoredefaultpath 
restoredefaultpath
bst_dir  = '/home/amit/git/ChildBrain/BeamComp/MATLAB/BeamComp_CodeRepo/brainstorm_181215/brainstorm3/';
bst_dir  = '/home/amit/Downloads/brainstorm3_master_26092019/brainstorm3-master/';

mri_dir  = '/net/qnap/data/rd/ChildBrain/BeamComp_DataRepo/MRI/';
data_dir = '/net/qnap/data/rd/ChildBrain/BeamComp_DataRepo/MEG/';
...
% for i_file = 25:64
    ...
    if ismember(i_file, 1:48) % Static phantom
        if ~isempty(strfind(filename, 'Amp20_'))
            data_path = [data_dir 'Phantom/Static_phantom/Amp20_IASoff/'];
        end
        if ~isempty(strfind(filename, 'Amp200_'))
            data_path = [data_dir 'Phantom/Static_phantom/Amp200_IASoff/'];
        end
        if ~isempty(strfind(filename, 'Amp1000_'))
            data_path = [data_dir 'Phantom/Static_phantom/Amp1000_IASoff/'];
        end
    elseif ismember(i_file, 49:64) % Moving phantom
        if ~isempty(strfind(filename, '_movement'))
            data_path = [data_dir 'Phantom/Moving_phantom/Amp200_IASoff_movement/'];
        end
    end
    ...
    grand_pow    = mean(stc,2); %% similar to FieldTrip || SPM12 (finding the max of the summed series)
    ...
    diary([code_dir 'diary_BST_Phantom_results.txt'])
    fprintf('%s,\t %.1f, %.1f, %.1f,\t %.1f, \t%.1f, \t%.1f, \t%.1f \n', dfname, Est_locNM, Est_val, difff, n_act_grid, PSVol)
    diary('off')
