...
fname    = data_dir + 'Simulated_evoked_30nAm_at_-4.4_-15.9_122.9mm-ave_OVER_spont_eo_raw_sss.fif'
...
if more_plots:
    mne.viz.plot_bem(subject=subject, subjects_dir=subjects_dir, orientation='coronal', slices=None, 
                 brain_surfaces='pial', src=src_vol, show=True)
    mne.viz.plot_alignment(epochs.info, trans, subject=subject, subjects_dir=subjects_dir, fig=None,
...
if more_plots:
    ...
    evoked.plot_topo()
evoked_pst = evoked.copy().crop(tmin=-0.2, tmax=-0.02)
 
noise_cov = mne.compute_covariance(epochs, tmin=-0.2, tmax=-0.02, method='empirical') 
data_cov  = mne.compute_covariance(epochs, tmin=0.02, tmax=0.2,   method='empirical')   
